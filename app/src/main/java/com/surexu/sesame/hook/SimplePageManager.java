...
public class SimplePageManager {
    ...
    public interface ActivityFocusHandler {
        boolean handleActivity(Activity activity, com.surexu.sesame.hook.SimpleViewImage root);
    }
    ...
    public static com.surexu.sesame.hook.SimpleViewImage tryGetTopView(String xpath) {
        // 清理空引用
        ...
        for (WeakReference<Dialog> dialogWeakReference : dialogs) {
            ...
            com.surexu.sesame.hook.SimpleViewImage viewImage = new com.surexu.sesame.hook.SimpleViewImage(decorView);
            ArrayList<com.surexu.sesame.hook.SimpleViewImage> results = (ArrayList<SimpleViewImage>) SimpleXpathParser.evaluate(viewImage, xpath);
            if (!results.isEmpty()) {
